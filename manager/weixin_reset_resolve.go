...
func (m *Manager) resolvePodWeixinReset(c *gin.Context) {
	...
	err := m.wdb.Db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&pod, "id = ? AND weixin_reset_pending = ?", c.Param("id"), true).Error; err != nil {
			return err
		}
		var bot schema.WeixinBot
		if err := tx.First(&bot, "id = ? AND user_id = ? AND assigned_pod_id = ? AND status IN ?", input.BotID, pod.UserID, pod.ID, []string{"reset_reserved", schema.WeixinBotStatusAssigned}).Error; err != nil {
			return errors.New("bot is not reserved by this pod")
		}
		if err := tx.Model(&schema.WeixinBot{}).Where("assigned_pod_id = ? AND id <> ?", pod.ID, bot.ID).Updates(map[string]any{"status": "retired", "assigned_pod_id": nil}).Error; err != nil {
			return err
		}
		if err := tx.Model(&bot).Update("status", schema.WeixinBotStatusAssigned).Error; err != nil {
			return err
		}
