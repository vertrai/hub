...
func (m *Manager) router() *gin.Engine {
	...
	r.GET("/v1/wechat/catalog/:id", m.getAgentCatalogEntry)
	r.GET("/v1/wechat/agents", m.listMyMiniProgramAgents)
	...
	r.POST("/v1/wechat/agents/:taskId/weixin-rebind", m.startMiniProgramRebind)
	...
	admin.DELETE("/agent-catalog/:id", m.adminDeleteAgentCatalog)
	admin.GET("/xbox/children", m.adminXboxChildren)
	admin.POST("/xbox/children", m.adminXboxChildren)
	admin.PATCH("/xbox/children/:id", m.adminUpdateXboxChild)
	admin.DELETE("/xbox/children/:id", m.adminDeleteXboxChild)
	r.GET("/v1/xbox-child", m.getXboxChild)
	r.POST("/v1/xbox-child", m.getXboxChild)
	...
	admin.GET("/llm/user-options", m.listLLMUserOptions)
	...
	admin.POST("/hymatrix/pods/:id/weixin/reset/resolve", m.resolvePodWeixinReset)
