...
func TestResetWeixinSendsCommandAsEncryptedData(t *testing.T) {
	...
	assertTagsEqual(t, fake.startPlain, map[string]string{"Action": "Eval"})
	if len(fake.startEncrypted) != 1 || fake.startEncrypted[0].Name != "Eval-Data" || !strings.Contains(fake.startEncrypted[0].Value, "hermes gateway restart") || !strings.Contains(fake.startEncrypted[0].Value, "base64 -d") {
